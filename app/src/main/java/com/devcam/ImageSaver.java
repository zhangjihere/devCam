...
class ImageSaver implements Runnable {
    public final static String APP_TAG = "ImageSaver";
    ...
    @Override
    public void run() {
        Log.v(APP_TAG, mImage.getFormat() + " ImageSaver.run()  image w/timestamp: " + SystemClock.elapsedRealtime());

        // Make sure we have a directory to save the image to.
        if (!(SAVE_DIR.mkdir() || SAVE_DIR.isDirectory())) {
            Log.v(APP_TAG, "Could not create/verify image directory " + SAVE_DIR);
        ...
        try (FileOutputStream output = new FileOutputStream(file)) {
            switch (mImage.getFormat()) {
                ...
                case ImageFormat.YUV_420_888:
                    ...
                    break;
                default:
                    Log.v(APP_TAG, "ImageSaver: format not recognized.");
            }
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            Log.v(APP_TAG, "Freeing buffer of image w/timestamp: " + SystemClock.elapsedRealtime());
